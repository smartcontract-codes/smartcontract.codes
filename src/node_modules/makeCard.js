...
function makeCard (contract) {
  ...
  const card = bel`
    <div class=${css.collectionCard}>
      <pre class=${css.code} onclick=${() => openInEditor(contract)}><code>${source}</code></pre>
...
var counter = 1
var _contract = void 0

window.addEventListener('message', event => {
  if (event.source === window.editor) {
    const [id, from, path, ref, type, body] = event.data
    if (!(type === 'ready' && _contract)) return console.error('unexpected message', {id, from, path, ref, type, body})
